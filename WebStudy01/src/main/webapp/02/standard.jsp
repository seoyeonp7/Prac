...
<pre>
	...
	JSP 소스 표준 구성 요소
	1. 정적 테스트 : 일반텍스트, HTML, JavaScript, CSS
		...
		1) scriptlet : 
 		...
 		2) directive : <%--<%@ page import="java.util.Date" %> --%>
 				: JSP 페이지에 대한 부가설정이나 전처리 구문에 사용되며,
 				 ...
 				 include(optional) : 정적 includep
 				 taglib(optional) : 커스텀 태그 로딩.
		3) expression : <%=data %>, <%=now %>
		4) declaration : 
		5) comment : <%-- --%>
			- client side comment : HTML, JS, CSS <!-- 응답 데이터 안에 주석처리가 표현 됨 -->
<!-- 			comment -->
			- server side comment : java, jsp <!-- 응답 데이터 안에 주석처리가 표현 안됨-->
			<% // java comment %>
			<%-- jsp comment --%>
	3. action tag
	4. EL(표현언어)
