...
<pre>
	...
	4. Connection 수립
	5. 쿼리 객체 생성
		- Statement : 가장 기본적인 쿼리 객체로 런타임에 쿼리가 동적 변경될 수 있음. sql injection에 취약함. -> 웬만하면 쓰지 마세요
		- PreparedStatement : 쿼리 파라미터를 포함하여, 쿼리 객체시 쿼리를 미리 컴파일함.
		- CallableStatement : function/procedure 와 같은 절차적 코드 집합을 실행할때 사용하는 쿼리 객체(function은 반환데이터가 존재, procedure은 반환데이터가 존재X(그래서 outbound를 사용함))
	6. 쿼리 실행
